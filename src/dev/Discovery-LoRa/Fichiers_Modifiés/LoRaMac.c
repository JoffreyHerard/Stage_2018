...
static uint8_t* LoRaMacAppNonce;
    ...
    switch( macHdr.Bits.MType )
    {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            if( micRx == mic )
            {
                ...
                LoRaMacAppNonce= LoRaMacRxPayload;
    ...
    if( LoRaMacState != LORAMAC_IDLE )
    {
    	...
    	switch(LoRaMacState){
    	 ...
    	 case  LORAMAC_RX:
    		 PRINTF("LORAMAC_RX");
		 	 break;
    	 case  LORAMAC_ACK_REQ:
    		 PRINTF("LORAMAC_ACK_REQ");
		 	 break;
...
void toggleSessionElement(char * messageAnswer){
	...
	PRINTF("Key ");
	PRINTF(" %02X", LoRaMacAppKey[0]) ;for(int i=1; i<16; i++) {PRINTF("%02X", LoRaMacAppKey[i]); memset( buffer, '\0', sizeof(char)*124 );sprintf(buffer, "%02X",LoRaMacAppKey[i]);strcat(messageAnswer,buffer); }; PRINTF("\n\n\r");
	PRINTF("\n\r");
	strcat(messageAnswer,":");

	PRINTF("AppNonce ");
	memset( buffer, '\0', sizeof(char)*124 );
	sprintf(buffer, "%02X",LoRaMacAppNonce[0]);
	strcat(messageAnswer,buffer);
	PRINTF(" = %02X", LoRaMacAppNonce[0]);for(int i=1; i<8; i++) {PRINTF("%02X", LoRaMacAppNonce[i]); memset( buffer, '\0', sizeof(char)*124 );sprintf(buffer, "%02X",LoRaMacAppNonce[i]);strcat(messageAnswer,buffer); }; PRINTF("\n\n\r");
	PRINTF("\n\r");
	strcat(messageAnswer,":");

	PRINTF("DevNonce ");
	memset( buffer, '\0', sizeof(char)*124 );
	sprintf(buffer, "%02X",LoRaMacDevNonce);
	strcat(messageAnswer,buffer);
	PRINTF(" = %02X", LoRaMacDevNonce);
	PRINTF("\n\r");
	strcat(messageAnswer,":");

	PRINTF("NwkSKey ");
	memset( buffer, '\0', sizeof(char)*124 );
	sprintf(buffer, "%02X",LoRaMacNwkSKey[0]);
	strcat(messageAnswer,buffer);
	PRINTF(" = %02X", LoRaMacNwkSKey[0]) ;for(int i=1; i<16; i++) {PRINTF("%02X", LoRaMacNwkSKey[i]); memset( buffer, '\0', sizeof(char)*124 );sprintf(buffer, "%02X",LoRaMacNwkSKey[i]);strcat(messageAnswer,buffer); }; PRINTF("\n\n\r");
	PRINTF("\n\r");
	strcat(messageAnswer,":");
	PRINTF("AppSKey ");

	memset( buffer, '\0', sizeof(char)*124 );
	sprintf(buffer, "%02X",LoRaMacAppSKey[0]);
	strcat(messageAnswer,buffer);
	PRINTF(" = %02X", LoRaMacAppSKey[0]) ;for(int i=1; i<16; i++) {PRINTF("%02X", LoRaMacAppSKey[i]); memset( buffer, '\0', sizeof(char)*124 );sprintf(buffer, "%02X",LoRaMacAppSKey[i]);strcat(messageAnswer,buffer); }; PRINTF("\n\n\r");
	PRINTF("\n\r");
	strcat(messageAnswer,":");
