...
//uint8_t* convertible(char *s, int length); // ok

void convertible(char *s, int length, uint8_t* ret);
void convertible32(char *s, int length,uint32_t* ret);
//uint32_t* convertible32(char *s, int length);//ok
//void freeArrayint8(uint8_t* array, int length); //ok
//void freeArrayint32(uint32_t* array, int length);//ok
...
int main( void )
{
	if(0){
		...
		/*testé*/
		uint8_t* newLoRaMacAppKey=(uint8_t*) malloc(sizeof(uint8_t)*16);
		convertible(tab[0],32,newLoRaMacAppKey);
		uint8_t* newLoRaMacAppNonce=(uint8_t*) malloc(sizeof(uint8_t)*8);
		convertible(tab[1],16,newLoRaMacAppNonce);
		uint16_t newLoRaMacDevNonce=strtoul(tab[2], NULL, 16);
		uint8_t* newLoRaMacNwkSKey=(uint8_t*) malloc(sizeof(uint8_t)*16);
		convertible(tab[3],32,newLoRaMacNwkSKey);
		uint8_t* newLoRaMacAppSKey=(uint8_t*) malloc(sizeof(uint8_t)*16);
		convertible(tab[4],32,newLoRaMacAppSKey);
		uint32_t newLoRaMacDevAddr=strtoul(tab[5], NULL, 16);
		uint32_t newmicRxsave =strtoul(tab[6], NULL, 16);
		/*EN TEST*/
		/*FIN TEST*/


		int tour= 0;
		char* elementSession=(char*)malloc(sizeof(char)*140);
		toggleSessionElement(elementSession);
		saveSessionElement();
		...
		free(newLoRaMacDevNonce);
		free(newLoRaMacDevAddr);
		free(newmicRxsave);
		free(newLoRaMacAppKey);
		free(newLoRaMacAppNonce);
		free(newLoRaMacNwkSKey);
		free(newLoRaMacAppSKey);
	}
...
void convertible(char *s, int length, uint8_t* ret){
	//int taille=length/2;
	DBG_PRINTF("BEGIN CONVERTIBLE 8 \n\r");
	//ret= (uint8_t*) malloc(sizeof(uint8_t)*taille);
	DBG_PRINTF("POST MALLOC CONVERTIBLE 8 \n\r");
	int index=0;
	...
	DBG_PRINTF("END CONVERTIBLE 8 \n\r");
}

void convertible16(char *s, int length, uint16_t* ret){
	//int taille=length/2;
	DBG_PRINTF("BEGIN CONVERTIBLE 8 \n\r");
	//ret= (uint8_t*) malloc(sizeof(uint8_t)*taille);
	DBG_PRINTF("POST MALLOC CONVERTIBLE 8 \n\r");
	...
	for(int i =0 ;i<length;i+=2){
		...
		char* substring =buff;
		//PRINTF("substring %s \r\n",substring);
		uint16_t val = strtoul(substring, NULL, 16);
		ret[index]=val;
		index++;
	}
	/*for(int i=0;i<16;i++){
...
void convertible32(char *s, int length,uint32_t* ret){
	//int taille=length/2	 ;
	DBG_PRINTF("CONVERTIBLE 32 \n\r");
	//ret= (uint32_t*) malloc(sizeof(uint32_t)*taille);
	DBG_PRINTF("POST MALLOC CONVERTIBLE 32 \n\r");
	int index=0;
	for(int i =0 ;i<length;i+=2){
		char buff[3];
		memcpy(buff, &s[i],2);
